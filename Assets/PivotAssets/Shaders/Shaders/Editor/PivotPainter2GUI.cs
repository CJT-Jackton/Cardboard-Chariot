...
{
    public abstract class PivotPainterGUI : ShaderGUI
    {
        ...
        protected class Styles
        {
            ...
            public static readonly GUIContent PivotPainterInputs = new GUIContent("Pivot Painter Inputs",
                "These settings control the pivot of the surface.");
            ...
            public static readonly GUIContent xVectorTex =
                new GUIContent("X-Vector Map", "Specifies the X-Vector and X-Extent Texture.");

            public static readonly GUIContent pivotPosTex =
                new GUIContent("Position Map", "Specifies the Pivot Position and Index Texture.");

            public static readonly GUIContent pivotPosScale =
                new GUIContent("Position Scale", "Specifies the Pivot Position Scale.");

            public static readonly GUIContent enableLevel1 =
                new GUIContent("Enable Level 1", "Enable the animation of the first level (trunk).");

            public static readonly GUIContent enableLevel2 =
                new GUIContent("Enable Level 2", "Enable the animation of the second level.");

            public static readonly GUIContent enableLevel3 =
                new GUIContent("Enable Level 3", "Enable the animation of the third level.");

            public static readonly GUIContent enableLevel4 =
                new GUIContent("Enable Level 4", "Enable the animation of the fourth level.");

            public static readonly GUIContent maxRotation1 =
                new GUIContent("Max Rotation 1", "Maximum amount of rotation [0, 1], a value of 1 represents 360 degrees.");

            public static readonly GUIContent maxRotation2 =
                new GUIContent("Max Rotation 2", "Maximum amount of rotation [0, 1], a value of 1 represents 360 degrees.");

            public static readonly GUIContent maxRotation3 =
                new GUIContent("Max Rotation 3", "Maximum amount of rotation [0, 1], a value of 1 represents 360 degrees.");

            public static readonly GUIContent maxRotation4 =
                new GUIContent("Max Rotation 4", "Maximum amount of rotation [0, 1], a value of 1 represents 360 degrees.");
        ...
        // Pivot Painter Input properties

        protected MaterialProperty xVectorTexProp { get; set; }

        protected MaterialProperty pivotPosTexProp { get; set; }

        protected MaterialProperty pivotPosScaleProp { get; set; }

        protected MaterialProperty enableLevel1Prop { get; set; }

        protected MaterialProperty enableLevel2Prop { get; set; }

        protected MaterialProperty enableLevel3Prop { get; set; }

        protected MaterialProperty enableLevel4Prop { get; set; }

        protected MaterialProperty maxRotation1Prop { get; set; }

        protected MaterialProperty maxRotation2Prop { get; set; }

        protected MaterialProperty maxRotation3Prop { get; set; }

        protected MaterialProperty maxRotation4Prop { get; set; }
        ...
        bool m_PivotPainterInputsFoldout;
        ...
        public virtual void FindProperties(MaterialProperty[] properties)
        {
            ...
            xVectorTexProp = FindProperty("_XVectorTex", properties, false);
            pivotPosTexProp = FindProperty("_PivotPosTex", properties, false);
            pivotPosScaleProp = FindProperty("_PivotPosScale", properties, false);
            enableLevel1Prop = FindProperty("_Enable_Level1", properties, false);
            enableLevel2Prop = FindProperty("_Enable_Level2", properties, false);
            enableLevel3Prop = FindProperty("_Enable_Level3", properties, false);
            enableLevel4Prop = FindProperty("_Enable_Level4", properties, false);
            maxRotation1Prop = FindProperty("_MaxRotation1", properties, false);
            maxRotation2Prop = FindProperty("_MaxRotation2", properties, false);
            maxRotation3Prop = FindProperty("_MaxRotation3", properties, false);
            maxRotation4Prop = FindProperty("_MaxRotation4", properties, false);
        ...
        public virtual void OnOpenGUI(Material material, MaterialEditor materialEditor)
        {
            ...
            m_PivotPainterInputsFoldout = new SavedBool($"{m_HeaderStateKey}.PivotPainterInputsFoldout", true);
            ...
            */
            ...
            m_PivotPainterInputsFoldout = true;
            m_SurfaceInputsFoldout = true;
            m_AdvancedFoldout = true;
        ...
        public void ShaderPropertiesGUI(Material material)
        {
            ...
            m_PivotPainterInputsFoldout = EditorGUILayout.BeginFoldoutHeaderGroup(m_PivotPainterInputsFoldout, Styles.PivotPainterInputs);
            if (m_PivotPainterInputsFoldout)
            {
                DrawPivotPainterInputs(material);
                EditorGUILayout.Space();
            }
            EditorGUILayout.EndFoldoutHeaderGroup();
        ...
        public virtual void DrawPivotPainterInputs(Material material)
        {
            DrawPivotPainterProperties(material);
            EditorGUILayout.Space();
            DrawPivotPainterLevelProperties(material);
        }
        ...
        public virtual void DrawPivotPainterProperties(Material material)
        {
            materialEditor.TexturePropertySingleLine(Styles.xVectorTex, xVectorTexProp);
            materialEditor.TexturePropertySingleLine(Styles.pivotPosTex, pivotPosTexProp);
            materialEditor.ShaderProperty(pivotPosScaleProp, Styles.pivotPosScale);
        }

        public virtual void DrawPivotPainterLevelProperties(Material material)
        {
            EditorGUI.BeginChangeCheck();
            EditorGUI.showMixedValue = enableLevel1Prop.hasMixedValue;
            var level1Enabled = EditorGUILayout.Toggle(Styles.enableLevel1, enableLevel1Prop.floatValue == 1);
            if (EditorGUI.EndChangeCheck())
                enableLevel1Prop.floatValue = level1Enabled ? 1 : 0;
            EditorGUI.showMixedValue = false;

            EditorGUI.BeginDisabledGroup(!level1Enabled);
            EditorGUI.indentLevel += 1;
            materialEditor.ShaderProperty(maxRotation1Prop, Styles.maxRotation1);
            EditorGUI.indentLevel -= 1;
            EditorGUI.EndDisabledGroup();
            EditorGUILayout.Space();

            EditorGUI.BeginChangeCheck();
            EditorGUI.showMixedValue = enableLevel2Prop.hasMixedValue;
            var level2Enabled = EditorGUILayout.Toggle(Styles.enableLevel2, enableLevel2Prop.floatValue == 1);
            if (EditorGUI.EndChangeCheck())
                enableLevel2Prop.floatValue = level2Enabled ? 1 : 0;
            EditorGUI.showMixedValue = false;

            EditorGUI.BeginDisabledGroup(!level2Enabled);
            EditorGUI.indentLevel += 1;
            materialEditor.ShaderProperty(maxRotation2Prop, Styles.maxRotation2);
            EditorGUI.indentLevel -= 1;
            EditorGUI.EndDisabledGroup();
            EditorGUILayout.Space();

            EditorGUI.BeginChangeCheck();
            EditorGUI.showMixedValue = enableLevel3Prop.hasMixedValue;
            var level3Enabled = EditorGUILayout.Toggle(Styles.enableLevel3, enableLevel3Prop.floatValue == 1);
            if (EditorGUI.EndChangeCheck())
                enableLevel3Prop.floatValue = level3Enabled ? 1 : 0;
            EditorGUI.showMixedValue = false;

            EditorGUI.BeginDisabledGroup(!level3Enabled);
            EditorGUI.indentLevel += 1;
            materialEditor.ShaderProperty(maxRotation3Prop, Styles.maxRotation3);
            EditorGUI.indentLevel -= 1;
            EditorGUI.EndDisabledGroup();
            EditorGUILayout.Space();

            EditorGUI.BeginChangeCheck();
            EditorGUI.showMixedValue = enableLevel4Prop.hasMixedValue;
            var level4Enabled = EditorGUILayout.Toggle(Styles.enableLevel4, enableLevel4Prop.floatValue == 1);
            if (EditorGUI.EndChangeCheck())
                enableLevel4Prop.floatValue = level4Enabled ? 1 : 0;
            EditorGUI.showMixedValue = false;

            EditorGUI.BeginDisabledGroup(!level4Enabled);
            EditorGUI.indentLevel += 1;
            materialEditor.ShaderProperty(maxRotation4Prop, Styles.maxRotation4);
            EditorGUI.indentLevel -= 1;
            EditorGUI.EndDisabledGroup();
        }
        ...
        public static void SetMaterialKeywords(Material material, Action<Material> shadingModelFunc = null, Action<Material> shaderFunc = null)
        {
            ...
            // Pivot Painter
            if (material.HasProperty("_Enable_Level1"))
                CoreUtils.SetKeyword(material, "_ENABLE_LEVEL1_ON", material.GetFloat("_Enable_Level1") == 1.0f);
            if (material.HasProperty("_Enable_Level2"))
                CoreUtils.SetKeyword(material, "_ENABLE_LEVEL2_ON", material.GetFloat("_Enable_Level2") == 1.0f);
            if (material.HasProperty("_Enable_Level3"))
                CoreUtils.SetKeyword(material, "_ENABLE_LEVEL3_ON", material.GetFloat("_Enable_Level3") == 1.0f);
            if (material.HasProperty("_Enable_Level4"))
                CoreUtils.SetKeyword(material, "_ENABLE_LEVEL4_ON", material.GetFloat("_Enable_Level4") == 1.0f);
